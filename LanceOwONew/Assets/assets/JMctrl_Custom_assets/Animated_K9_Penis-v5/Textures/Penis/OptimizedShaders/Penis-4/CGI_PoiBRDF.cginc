...
void poiBRDF(inout float4 finalColor, const float4 finalColorBeforeLighting)
{
    ...
    float metallic = metallicGlossMap.r * float(0.217);
    float reflectance = metallicGlossMap.g * float(0.804);
    float roughness = max(1 - (float(0.88) * metallicGlossMap.a), getGeometricSpecularAA(poiMesh.normals[1]));
