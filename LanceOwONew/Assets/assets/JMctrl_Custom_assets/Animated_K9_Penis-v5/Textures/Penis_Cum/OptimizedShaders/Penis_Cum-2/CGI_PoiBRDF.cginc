...
void poiBRDF(inout float4 finalColor, const float4 finalColorBeforeLighting)
{
    ...
    float metallic = metallicGlossMap.r * float(0.225);
    float reflectance = metallicGlossMap.g * float(0.768);
    float roughness = max(1 - (float(0.831) * metallicGlossMap.a), getGeometricSpecularAA(poiMesh.normals[1]));
