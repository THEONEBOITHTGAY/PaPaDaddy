...
void initTextureData(inout float4 albedo, inout float4 mainTexture, inout float3 backFaceEmission, inout float3 dissolveEmission, in half3 detailMask)
{
    ...
    mainTexture.a *= alphaMask;
        ...
        albedo = float4(mainTexture.rgb * max(float4(0.1653383,0.01741692,0.07941269,1).rgb, float3(0.000000001, 0.000000001, 0.000000001)) * lerp(1, vertexColor, float(0)), mainTexture.a * max(float4(0.1653383,0.01741692,0.07941269,1).a, 0.0000001));
        ...
        #endif
        albedo.a = saturate(float(0.51) + albedo.a);
        wireframeEmission = 0;
        #ifdef POI_WIREFRAME
